...
class rgtn_b(nn.Module):
    ...
    def forward(self, blocks, struct_input, content_input, labels=None):
        ...
        # 训练
        if self.training:
            loss_struct = self.loss_fn(logit_struct, labels)       # 结构的MSE损失L1
            loss_content = self.loss_fn(logit_content, labels)     # 语义的MSE损失L2
            loss_all = self.loss_fn(logits, labels)                # 两者的MSE损失L0
            loss = (1-self.loss_lambda) * loss_all + self.loss_lambda * (loss_struct + loss_content) / 2   # 添加无监督的损失？ 0.5  L0+(L1+L2)/2
            loss = self.loss_alpha * list_loss(logits, labels, self.list_num) + loss   # 总损失   L0+(L1+L2)/2+Lj
