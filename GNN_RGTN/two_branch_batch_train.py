...
from metric import overlap, ndcg
from rgtn_model import rgtn_b
...
def main(args):
    ...
    for cross_id in range(args.cross_num):     # 交叉验证
        ...
        # 得到图、边类型、关系数量、初始结构特征、初始语义特征、标签、训练节点、验证节点、测试节点
        ...
        # create model
        loss_fcn = torch.nn.MSELoss()  # 均方损失
        model = rgtn_b(args, rel_num, num_struct_feat, num_content_feat, loss_fcn)   # 加载模型
        ...
        times.append(total_time / args.epochs)
    ...
    # print(times, times.mean(), np.std(times))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset", type=str, default='TMDB_rel_two',
                        help="The input dataset. Can be TMDB_rel")
