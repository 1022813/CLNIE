    ...
    for cross_id in range(args.cross_num):
        ...
        for epoch in range(args.epochs):
            ...
            if epoch >= 3:
                t0 = time.time()
            # forward
            logits, loss = model(struct_feat, content_feat, edge_types, labels, train_idx)
            ...
            model.eval()
            with torch.no_grad():
                val_logits = model(struct_feat, content_feat, edge_types)
                _, val_ndcg, val_spm = rank_evaluate(val_logits[val_idx], labels[val_idx].unsqueeze(-1), 100, loss_fcn, True)
                test_loss, test_ndcg, test_spm = rank_evaluate(val_logits[test_idx], labels[test_idx].unsqueeze(-1), 100, loss_fcn, True)
        ...
        model.eval()
        with torch.no_grad():
            test_logits = model(struct_feat, content_feat, edge_types)
